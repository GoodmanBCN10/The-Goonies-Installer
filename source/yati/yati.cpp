...
struct Yati {
    ...
    s64 m_total_package_size{};
    s64 m_accumulated_offset{};
    ...
    while (t->decompress_offset < (is_ncz ? t->write_size.load() : t->nca->size) && R_SUCCEEDED(t->GetResults())) {
        ...
        if (!is_ncz || !decompress_buf_off) {
            ...
            if (!decompress_buf_off) {
                ...
                if (t->write_size > t->nca->size) {
                    m_total_package_size += (t->write_size - t->nca->size);
                }
...
Result InstallInternal(source::Base* source, const container::Collections& collections, const ConfigOverride& override) {
    ...
    yati->m_total_package_size = 0;
    yati->m_accumulated_offset = 0;
    for (const auto& collection : collections) {
        if (collection.name.ends_with(".nca") || collection.name.ends_with(".ncz") || collection.name.ends_with(".tik") || collection.name.ends_with(".cert")) {
            yati->m_total_package_size += collection.size;
        }
    }
